...
std::string MemorySegment::Name() const { return name_.GetName(); }
...
absl::StatusOr<uint8_t*> MemorySegment::Get(const MemoryName& name,
                                            size_t segment_size) {
  auto shm_fd = shm_open(name.GetName(), O_RDWR, kShmMode);
  if (shm_fd == -1) {
...
MemorySegment::MemorySegment(const MemoryName& name, uint8_t* segment)
    : name_(name),
...
MemorySegment::MemorySegment(MemorySegment&& other) noexcept
    : name_(std::exchange(other.name_, MemoryName("", ""))),
