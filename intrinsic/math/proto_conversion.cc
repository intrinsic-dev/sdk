...
static bool AlmostEquals(const double x, const double y) {
  ...
  if (abs_x <= kStdError && abs_y <= kStdError) return true;

  if (std::isinf(x) || std::isnan(x) || std::isinf(y) || std::isnan(y)) {
    return false;
  }

  const double relative_margin = kStdError * std::max(abs_x, abs_y);
  const double max_error = std::max(kStdError, relative_margin);
